...
import daily_train
from daily_train.adapter import GPT as BaseModel
from daily_train.adapter import Block as BaseBlock
from daily_train.adapter import CausalSelfAttention as BaseCausalSelfAttention
from daily_train.adapter import Config as BaseConfig
from daily_train.model import KVCache
from daily_train.utils import map_old_state_dict_weights
...
class Config(BaseConfig):
    ...
    def mlp_class(self) -> Type:
        return getattr(daily_train.adapter_v2, self._mlp_class)
...
class Block(BaseBlock):
    """The implementation is identical to `daily_train.model.Block` with the exception that
...
class CausalSelfAttention(BaseCausalSelfAttention):
    """A modification of `daily_train.adapter.CausalSelfAttention` that uses the Adapter V2 Linear class"""
...
class GptNeoxMLP(daily_train.model.GptNeoxMLP):
    def __init__(self, config: Config) -> None:
        nn.Module.__init__(self)
...
class LLaMAMLP(daily_train.model.LLaMAMLP):
    def __init__(self, config: Config) -> None:
        nn.Module.__init__(self)
