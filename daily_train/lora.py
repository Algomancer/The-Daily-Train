...
import daily_train
from daily_train.config import Config as BaseConfig
from daily_train.model import GPT as BaseModel
from daily_train.model import Block as BaseBlock
from daily_train.model import CausalSelfAttention as BaseCausalSelfAttention
...
class LoRAQKVLinear(LoRALinear):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            n_query_groups: number of query groups (see diagram in `daily_train/config.py`)
            r: rank of the weight update matrices. To make sense of using LoRA the rank should be smaller than the rank of
                the weights of the model. The rank can be as low as 1: https://arxiv.org/pdf/2106.09685.pdf (section 7.2)
            lora_alpha: alpha is needed for scaling updates as alpha/r
                "This scaling helps to reduce the need to retune hyperparameters when we vary r"
    ...
    def conv1d(self, input: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        ...
        (see scheme in `daily_train/config.py:Config`). In this case the combined QKV matrix consists of equally sized
...
class Config(BaseConfig):
    ...
    def mlp_class(self) -> Type:
        return getattr(daily_train.lora, self._mlp_class)
